...
const MUNDO_DA_NUVEM = "res://scenes/Scenario/AngryCloud.tscn"
...
func get_input():
	...
	var JUMP = Input.is_action_just_pressed("JUMP") if is_angry_cloud_game else false
	var UP = Input.is_action_pressed("UP") if not is_angry_cloud_game else false
	var DOWN = Input.is_action_pressed("DOWN") if not is_angry_cloud_game else false
